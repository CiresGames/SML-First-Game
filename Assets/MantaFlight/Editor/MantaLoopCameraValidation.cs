...
namespace MantaFlight.Editor
{
    public static class MantaLoopCameraValidation
    {
        public static string Run()
        {
            ...
            void Step()
            {
                manta.Simulate(default, .02f); Physics.Simulate(.02f); camera.Tick(.02f);
            }
            try
            {
                ...
                {
                    manta.ResetFlight(); body.position = new Vector3(0, 180, -210); Physics.SyncTransforms();
                    ...
                    bool visible = true;
                    int frames = Mathf.RoundToInt(manta.settings.loopDuration / .02f);
                    for (int i = 0; i < frames - 1; i++)
                    {
                        ...
                        if (progress > .2f)
                        {
                            Vector3 viewport = lens.WorldToViewportPoint(body.position);
                        ...
                        if (i == frames / 2) { holdPosition = camera.transform.position; holdRotation = camera.transform.rotation; }
                        if (i > frames / 2)
                        {
                            maximumDrift = Mathf.Max(maximumDrift, Vector3.Distance(holdPosition, camera.transform.position));
                        ...
                        {
                            manta.GetComponent<MantaVisuals>().SendMessage("LateUpdate");
                            Capture(lens, "loop-" + trick + "-" + i + ".png");
                        }
                    }
                    Check(visible, trick + " stays framed at " + speed + " m/s");
                    Check(maximumDrift < .25f && maximumRotation < 1,
                        trick + " camera holds still: " + maximumDrift.ToString("F3") + " m / " + maximumRotation.ToString("F2") + " degrees");
                    Check(minimumDistance > manta.settings.cameraDistance, trick + " keeps its distance (minimum " + minimumDistance.ToString("F1") + " m)");
                    Vector3 before = camera.transform.position;
                    Step();
                    Check(Vector3.Distance(before, camera.transform.position) < .5f, trick + " exit has no position snap");
