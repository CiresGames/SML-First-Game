...
namespace MantaFlight
{
    public sealed class MantaVisuals : MonoBehaviour
    {
        ...
        void LateUpdate()
        {
            ...
            clock += Time.deltaTime * s.wingFrequency * Mathf.Lerp(1.2f, .7f, controller.Speed01) * Mathf.PI * 2;
            float wave = Mathf.Sin(clock);
            float breath = wave * .12f * intensity;
            visualRoot.localPosition = new Vector3(0, breath, 0);
            Quaternion wanted = Quaternion.Euler(Mathf.Clamp(-controller.Acceleration * .13f, -4, 4) * intensity, 0,
                controller.Bank + maneuvers.VisualRoll);
            visualRoot.localRotation = maneuvers.Current == MantaTrick.None
                ? Quaternion.Slerp(visualRoot.localRotation, wanted, MantaFlightSettings.Damp(12, Time.deltaTime)) : wanted;
